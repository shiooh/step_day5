...
都市を座標に基づいて格子状に分割し、各都市を対応する領域へ割り当てる。領域内の都市数が少なすぎる（4つ未満）場合は隣接領域と統合することで、バグを防ぐ。

a. 座標平面を n×n に分割
b. 各都市を所属する領域に配置
c. 都市数が少ない領域は隣接領域と統合

### 2. 部分巡回路の構築（tsp_greedy()）
...
### 3. 2-optによる部分巡回路の改善（apply_2opt()）

貪欲法で作成した巡回路には交差が含まれることがあるため、2-opt法によって改善する。2本の辺を巡査し、交差していた場合辺を張り替えて巡回路長を短縮する。この処理を交差がなくなるまで最大10回繰り返す。

a. 2本の辺を巡査（apply_2opt_once()）
b. 交差していた場合辺を張り替えて交差を解消（swap_crossing_paths()）
c. 改善がなくなるまで最大10回繰り返す（apply_2opt()）

### 4. 巡回路の結合（joint_clusters()）
...
b. 距離増加が最も小さい接続方法を探索
c. 辺を交換して巡回路を結合
d. 全ての巡回路が1つになるまで繰り返す

### 5. 全体巡回路の改善（apply_2opt()）
...
| cluster | 3418.10 | 3832.29 | 4664.52 | 8975.61 | 11951.19 | 22457.25 | 47270.67 |
